"""
Phase 5 Figure 5 — individual panel output EXCLUDING LS-CRC3.
Left group: WGC + JCA only (n=2).
Outputs to .../Anatomical_Distribution/figures_no_lscrc3/
         and .../Anatomical_Distribution/tables_no_lscrc3/
"""
...
FDIR = pathlib.Path("Project_Delivery/results/Figure5_Anatomical_Heterogeneity"
                    "/Anatomical_Distribution/figures_no_lscrc3")
TDIR = pathlib.Path("Project_Delivery/results/Figure5_Anatomical_Heterogeneity"
                    "/Anatomical_Distribution/tables_no_lscrc3")
...
adata_full = ad.read_h5ad(OUT / "gse188711_processed.h5ad")

# Exclude LS-CRC3
adata = adata_full[adata_full.obs['sample'] != 'LS-CRC3'].copy()
print(f"Excluded LS-CRC3: {adata_full.n_obs} -> {adata.n_obs} cells")
...
ax_a.text(-8, u2.max()+0.5, 'LCRC\n(Left, n=2)',  ha='center', fontsize=9, fontweight='bold', color=C_LEFT)
ax_a.text( 8, u2.max()+0.5, 'RCRC\n(Right, n=3)', ha='center', fontsize=9, fontweight='bold', color=C_RIGHT)
ax_a.axvline(0, ls='--', color='#999999', lw=0.8)
ax_a.axis('off')
ax_a.set_title('A  Split-UMAP: LCRC vs RCRC\n(LS-CRC3 excluded)', fontsize=11, fontweight='bold', loc='left', pad=4)
...
ax_b.set_title('B  OLR1+ TAM Abundance by Sidedness\n(LS-CRC3 excluded)',
...
# ─────────────────────────────────────────────────────────────────────────────
# PANEL D: Pathway scores — recompute without LS-CRC3
# ─────────────────────────────────────────────────────────────────────────────
from scipy.sparse import issparse as _issparse

PATHWAY_GENES = {
    'T cell Exclusion':  ['TGFB1','TGFB2','CCL2','CXCL12','VEGFA','IDO1','ARG1','IL10','CD274'],
    'Matrix Remodeling': ['MMP2','MMP9','MMP14','FAP','ACTA2','COL1A1','POSTN','SPARC','FN1'],
    'Lipid Metabolism':  ['OLR1','LPL','FABP5','CYP27A1','APOE','PPARG','FASN','HMGCR','LDLR'],
    'Angiogenesis':      ['VEGFA','ANGPT2','PDGFB','FGF2','HIF1A','PECAM1','CDH5','KDR','NRP1'],
    'Immune Suppression':['IL10','TGFB1','ARG1','IDO1','PDCD1LG2','CD274','HAVCR2','LAG3','TIGIT'],
}

pw_rows = []
for pw_name, pw_genes in PATHWAY_GENES.items():
    genes_present = [g for g in pw_genes if g in adata_rn.var_names]
    if not genes_present:
        continue
    left_means, right_means = [], []
    for samp, side in SAMPLE_META.items():
        sub_obs = adata.obs[adata.obs['sample'] == samp]
        # high OLR1 cells
        high_idx = sub_obs[sub_obs['OLR1TAM_score'] > sub_obs['OLR1TAM_score'].quantile(0.70)].index
        if len(high_idx) < 3:
            continue
        sub_rn = adata_rn[high_idx]
        X = sub_rn[:, genes_present].X
        if _issparse(X): X = X.toarray()
        m = float(X.mean())
        if side == 'Left':
            left_means.append(m)
        else:
            right_means.append(m)
    if not left_means or not right_means:
        continue
    lm, rm = np.mean(left_means), np.mean(right_means)
    fc = np.log2((rm + 1e-6) / (lm + 1e-6))
    from scipy.stats import ttest_ind as _ttest
    _, pv = (_ttest(right_means, left_means) if (len(right_means)>1 and len(left_means)>1)
             else (0, 1.0))
    pw_rows.append({'pathway': pw_name, 'left_mean': round(lm,6),
                    'right_mean': round(rm,6), 'log2fc': round(fc,4),
                    'pval': round(float(pv),6)})

pw_df = pd.DataFrame(pw_rows)
pw_df['sig'] = pw_df['pval'].apply(
    lambda p: '***' if p<0.001 else '**' if p<0.01 else '*' if p<0.05 else 'n.s.')
pw_df.to_csv(TDIR / 'panel_D_pathway_scores.csv', index=False)
print(f"Saved panel_D_pathway_scores.csv -> {TDIR}")

pw_sorted = pw_df.sort_values('log2fc').copy()

fig_d, ax_d = plt.subplots(figsize=(6.5, 5))
fig_d.subplots_adjust(left=0.30, right=0.94, top=0.88, bottom=0.12)

bar_colors_d = [C_LEFT if v < 0 else C_RIGHT for v in pw_sorted.log2fc]
ax_d.barh(range(len(pw_sorted)), pw_sorted.log2fc, color=bar_colors_d, alpha=0.8,
...
ax_e.set_title('E  OLR1+ TAM Abundance vs TME Composition\n'
               '(per-sample Spearman, n=5, LS-CRC3 excluded)',
               fontsize=11, fontweight='bold', loc='left', pad=6)
...
ax_f.set_title('F  OLR1+ TAM Signature Strength vs MMRp Reference\n(LS-CRC3 excluded)',
               fontsize=11, fontweight='bold', loc='left', pad=6)
...
print("\nAll panels and tables saved (LS-CRC3 excluded).")
